...
public static class SceneSerializer
{
    ...
    private static ComponentDocument ToComponentDocument(Component component)
    {
        ComponentDocument document = component switch
        {
            ...
            ParticleSystem2D particles => new ComponentDocument("ParticleSystem2D", Speed: particles.EmissionRate, Width: particles.MaxParticles, PlaybackSpeed: particles.Lifetime, SpotAngle: particles.SpreadAngle, MoveSpeed: particles.StartSpeed, Radius: particles.StartSize, FontSize: particles.EndSize, GravityScale: particles.Gravity.X, Intensity: particles.Gravity.Y, Color: ToColorString(particles.StartColor), OutlineColor: ToColorString(particles.EndColor), SortingOrder: particles.SortingOrder, RenderLayer: particles.RenderLayer.ToString(), IsPlaying: particles.IsEmitting, IsKinematic: particles.Looping),
    ...
    private static Component CreateComponent(ComponentDocument document, AssetManager? assets)
    {
        return document.Type switch
        {
            ...
            "ParticleSystem2D" => new ParticleSystem2D { EmissionRate = document.Speed ?? 32.0f, MaxParticles = document.Width ?? 160, Lifetime = document.PlaybackSpeed ?? 1.25f, SpreadAngle = document.SpotAngle ?? 55.0f, StartSpeed = document.MoveSpeed ?? 140.0f, StartSize = document.Radius ?? 18.0f, EndSize = document.FontSize ?? 2.0f, Gravity = new Vector2(document.GravityScale ?? 0.0f, document.Intensity ?? 85.0f), StartColor = FromColorString(document.Color ?? "#F0FFD65C"), EndColor = FromColorString(document.OutlineColor ?? "#00FF5220"), SortingOrder = document.SortingOrder ?? 20, RenderLayer = ParseRenderLayer(document.RenderLayer), IsEmitting = document.IsPlaying ?? true, Looping = document.IsKinematic ?? true },
